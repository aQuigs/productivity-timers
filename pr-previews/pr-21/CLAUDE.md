...
- After the modal applies, every card that received time (`> 0` ms) gets the `time-added` class: `.timer-card.time-added::after` plays `time-added-ring` (an accent ring expanding out from the edge) and the display plays `time-added-bump` (a small scale/colour bump). The card's own `animationend` removes the class (the display's bump ends first and bubbles, so it is ignored); `App.#animateTimeAdded()` removes and re-adds the class around a forced reflow so a second allocation replays it, which also covers reduced motion, where no `animationend` fires and the inert class stays on the card
